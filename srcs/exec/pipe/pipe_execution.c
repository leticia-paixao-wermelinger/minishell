...
/*   Updated: 2024/10/09 20:16:43 by lraggio          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    run_pipe_execve(t_command *command, t_node *list)
{
    ...
    {
        if (errno == EACCES)
            return (close_node_fds(list), free_list(list), clear_input(command),
            free_matrix(env_array), print_errno(node), ERROR);
        path = get_executable_path(command, node);
        if (!path)
            return (close_node_fds(list), free_list(list), clear_input(command),
            free_matrix(env_array), ERROR);
    }
    else
